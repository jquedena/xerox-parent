...
$.validity = {
    ...
    messages:{
        ...
        isLengthVal1Val2: "Debe ingresar #{val1} \u00f3 #{val2} caracteres",
...
$.fn.extend({
    ...
    selectSelected:function(val, msg) {
        // If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	var o = $(this);
        	if (o.is("select") && o.find("option").filter("select option:selected").val() == val) {       	
	            raiseAggregateError(
	                o,
	                msg || $.validity.messages.radioChecked
	            );
    ...
    // Verifica si es multipo de un numero:
    ///////////////////////////////////////////////////////////////////////////
    
    isMultiplo:function(val, msg) {
    	// If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	var o = $(this);
        	if (o.is(":text") && parseFloat(o.filter(":text").val())%val != 0) {       	
	            raiseAggregateError(
	                o,
	                msg || ($.validity.messages.isMultiplo + val) 
	            );
	        }
        });
	},
    
    // Verifica un atributo en especifico:
    ///////////////////////////////////////////////////////////////////////////
    
    attrChecked:function(attr, val, msg) {
    	// If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	...
        	if (o.attr(attr) == val) {       	
	            raiseAggregateError(
	                o,
	                msg || ($.validity.messages.attrChecked) 
	            );
	        }
        });
	},
	
    // Verifica la longitud ingresada:
    ///////////////////////////////////////////////////////////////////////////
    
	isLength:function(val, msg) {
    	// If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	var o = $(this);
        	if (o.is(":text") && $.trim(o.filter(":text").val()).length != val) {       	
	            raiseAggregateError(
	                o,
	                msg || ($.validity.messages.isLength.replace("#{val}", val) ) 
	            );
	        }
        });
	},
	
    // Verifica la longitud ingresada sea mayor o igual:
    ///////////////////////////////////////////////////////////////////////////
    
	isMaxLength:function(min, max, msg) {
    	// If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	var o = $(this);
        	if (o.is(":text") && !($.trim(o.filter(":text").val()).length >= min && $.trim(o.filter(":text").val()).length <= max)) {
	            raiseAggregateError(
	                o,
	                msg || ($.validity.messages.isMaxLength.replace("#{min}", min).replace("#{max}", max) ) 
	            );
	        }
        });
	},
	
    // Verifica la longitud ingresada sea igual a val1 o val2:
    ///////////////////////////////////////////////////////////////////////////
    
	isLengthVal1Val2:function(val1, val2, msg) {
    	// If a reduced set is attached, use it. Also, remove unsupported 
        // elements.
        var $reduction =  (this.reduction || this).filter($.validity.settings.elementSupport);
        ...
        $reduction.each(function() {
        	var o = $(this);
        	if (o.is(":text") && !($.trim(o.filter(":text").val()).length == val1 || $.trim(o.filter(":text").val()).length == val2)) {       	
	            raiseAggregateError(
	                o,
	                msg || ($.validity.messages.isLengthVal1Val2.replace("#{val1}", val1).replace("#{val2}", val2) ) 
	            );
	        }
        });
	},	
...
(function($) {
    $.validity.outputs.tooltip = {
        ...
        raise:function($obj, msg) {
            var pos = $obj.offset(), $tmp = $obj;
            pos.left += $obj.width() + 18;
            pos.top += 8;
            
            if($obj.hasClass("pui-inputtext")) {
                $tmp = $obj.next();
            }

            $("<img class=\"validity-tooltip\">").insertAfter($tmp)
	    		.attr("src", obtenerContexto("public/img/exclamation_red_frame.png"))
	    		.attr("title", msg)
	            .tooltip({
					tooltipClass : "ui-state-error"
				});
            /*
            $("<div class=\"validity-tooltip\">" + "**F**" +
