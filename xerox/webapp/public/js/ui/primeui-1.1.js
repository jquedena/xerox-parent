    ...
    $.widget("primeui.puiautocomplete", {
        ...
        _create: function() {
            ...
            if(this.options.multiple) {
                ...
                if(this.options.dropdown) {
                    this.dropdown = $('<button type="button" class="pui-button pui-button-autocomplete ui-widget ui-button ui-state-default ui-corner-right pui-button-icon-only">' +
            ...
            if(this.options.dropdown) {
                this.dropdown.on('hover.puiautocomplete', function() {
                    if(!$this.element.prop('disabled')) {
                        // TODO: $this.dropdown.toggleClass('ui-state-hover');
                    }
                })
                .on('mousedown.puiautocomplete', function() {
                    if(!$this.element.prop('disabled')) {
                        // TODO: $this.dropdown.addClass('ui-state-active');
                    }
                })
                .on('mouseup.puiautocomplete', function() {
                    if(!$this.element.prop('disabled')) {
                        // TODO: $this.dropdown.removeClass('ui-state-active');
                        $this.search('');
                        $this.element.focus();
                    }
                })
                .on('focus.puiautocomplete', function() {
                    // TODO: $this.dropdown.addClass('ui-state-focus');
                    $this.dropdown.addClass('ui-text-highlight');
                })
                .on('blur.puiautocomplete', function() {
                    // TODO: $this.dropdown.removeClass('ui-state-focus');
                    $this.dropdown.removeClass('ui-text-highlight');
                })
        ...
        _bindKeyEvents: function() {
            ...
            this.element.on('keyup.puiautocomplete', function(e) {
                ...
                if($this.panel.is(':visible')) {
                    var keyCode = $.ui.keyCode,
                    highlightedItem = $this.items.filter('.pui-ui-state-highlight');
                    ...
                    switch(e.which) {
                        ...
                        case keyCode.LEFT:
                            ...
                            if(prev.length == 1) {
                                highlightedItem.removeClass('pui-ui-state-highlight');
                                prev.addClass('pui-ui-state-highlight');
                        ...
                        case keyCode.RIGHT:
                            ...
                            if(next.length == 1) {
                                highlightedItem.removeClass('pui-ui-state-highlight');
                                next.addClass('pui-ui-state-highlight');
        ...
        _bindDynamicEvents: function() {
            ...
            this.items.on('mouseover.puiautocomplete', function() {
                ...
                if(!item.hasClass('pui-ui-state-highlight')) {
                    $this.items.filter('.pui-ui-state-highlight').removeClass('pui-ui-state-highlight');
                    item.addClass('pui-ui-state-highlight');
                }
            })
                ...
                if($.isArray(this.options.completeSource)) {
                    ...
                    for(var i = 0 ; i < sourceArr.length; i++) {
                        ...
                        if(emptyQuery||itemLabel.indexOf(this.query) === 0) {
                        	if($.isPlainObject(sourceArr[i])) {
                        		data.push({label:sourceArr[i].label, value: item});
                        	} else {
        ...
        _handleData: function(data) {
            ...
            if(this.items.length > 0) {
                ...
                firstItem.addClass('pui-ui-state-highlight');
...
$(function() {
    ...
    $.widget("primeui.puiinputtext", {
        ...
        _create: function() {
            ...
            //visuals
            // TODO: Elimino CSS: ui-state-default ui-corner-all
            input.addClass('pui-inputtext ui-widget');
        ...
        _enableMouseEffects: function () {
            ...
            input.hover(function () {
                // TODO: input.toggleClass('ui-state-hover');
            }).focus(function () {
                // TODO: input.addClass('ui-state-focus');
            }).blur(function () {
                // TODO: input.removeClass('ui-state-focus');
            });
        },
